...
(function(){
    ...
    var gTable = window.gTable = function(id, options){
        ...
        // 安装表单提交事件监听器。在表单提交时，将gTable的数据置入与其同名的key中
        var setupSubmit = function(table){
            var parent = table.parent();

            if(parent.is('html') || parent.is('head') || parent.is('body'))
                return;

            if(parent.is('form')){
                parent.submit(function(){
                    $("<input name='"+$(id).attr("id")+"' value='"+JSON.stringify(options.data)+"' type='hidden'/>").appendTo(this);
                });

                return true;
            }
            
            return setupSubmit(parent);
        }
